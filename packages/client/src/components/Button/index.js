...
export const Button = React.forwardRef(
  (
    ...
    const styles = {};
    if (backgroundColor) {
      styles[variant === 'primary' ? 'backgroundColor' : 'color'] =
        backgroundColor;
    ...
    return (
      ...
      >
        {!!icon && !loading && (
          <div
            className={`${ButtonCss.iconImg} ${iconSize} ${
              label ? 'mr-2' : ''
            }`}
          >
            {icon}
        ...
        {!!tooltip && (
          <ReactTooltip place="bottom" id={tooltip} type="dark">
            <div className="text-center max-w-48">{tooltip}</div>
          </ReactTooltip>
        )}
        {loading && (
          <Spinner
            className={`${variant === 'outline' && 'themeSpinner'} ${
              size === 'medium' && 'spinnerMedium'
            } ${variant === 'ghost' && 'themeSpinner'}`}
          />
        )}
        <span className={`${loading && 'opacity-0'} ${wrapClass}`}>
          {label || children}
        </span>
      </button>
    );
