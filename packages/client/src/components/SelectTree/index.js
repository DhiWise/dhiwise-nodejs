...
export const SelectTree = memo(
  ({
    ...
    const nodeConversion = useCallback(
      (node) => ({
        fullName: node?._parentName
          ? `${node?._parentName}.${node?.label}`
          : node?.label,
        singleName: node?.label,
      }),
      []
    );
      ...
      <div className={WrapClassName}>
          ...
          <div className={`${LeftLabel}`}>
            {!!label && (
              <div className={labelClass}>
                <label className="mb-1.5 text-primary-text text-sm block w-full font-normal leading-5">
                  {label}
                </label>
              </div>
            )}
        ...
        <div className="SelectTreeDisable">
          <DropDownComponent
            ...
            data={data}
            changeRef={changeRef}
            className={`${className} ${dark && 'darkSelect'} ${
              size === 'small' && 'smallBox'
            }`}
            mode={mode}
        ...
        {!!desc && <Description className="mt-1">{desc}</Description>}
...
const DropDownComponent = memo(
  ({
    ...
    const recursiveTree = (recursiveData, parent = null) =>
      Object.keys(recursiveData)
        ?.map(
          (mainKey) =>
            ...
            typeof recursiveData[mainKey] === 'object' && {
              ...
              ...(!isEmpty(changeRef.current?.defaultValue)
                ? {
                    expanded: flatten(
                      changeRef.current?.defaultValue?.map((expand) => {
                        const parentArray = expand.split('.');
                        parentArray.push(
                          parentArray
                            .slice(0, expand.split('.').lastIndexOf('.'))
                            .join('.')
                        );
                        return parentArray;
                      })
              ...
              ...(!isEmpty(changeRef.current.defaultValue)
                ? {
                    isDefaultValue: changeRef.current.defaultValue?.includes(
                      parent ? `${parent}.${mainKey}` : mainKey
                    ),
                  }
                : {}),
              ...(!isEmpty(changeRef.current.disabledKey) && {
                disabled: changeRef.current.disabledKey?.includes(
                  parent ? `${parent}.${mainKey}` : mainKey
                ),
              }),
              ...(Object.values(recursiveData[mainKey])?.findIndex(
              ...
              ...(Array.isArray(recursiveData[mainKey])
                ? {
                    children: recursiveTree(
                      { ...recursiveData[mainKey]?.[0] },
                      parent ? `${parent}.${mainKey}` : mainKey
                    ),
                  }
                : {}), // If array key value pair
            }
        )
        .filter((el) => el && el);
    ...
    return (
      <DropdownTreeSelect
        ...
        onFocus={() => {
          setData(
            isEmpty(customDataJson) ? recursiveTree(data) : customDataJson
          );
        }}
        mode={mode}
