...
export const Input = forwardRef(
  (
    ...
    const handleChange = (e) => {
      ...
      if (customRegex) {
        if (!targetVal || customRegex.test(targetVal))
      ...
      <div className={[wrapperClasses, WrapClassName].join(' ')}>
        {!!(label || desc) && (
          <div className={labelClassName}>
            {!!label && (
              <label className={`${InputCss.inputLabel}`}>{label}</label>
            )}
          </div>
        )}
        <div className={`${className} ${InputCss.inputWrapperClass}`}>
          <div className="relative">
            <input
              ...
              className={`${
                InputCss.inputBlock
              } truncate ${inputClassName} ${inputClass} ${sizeCss} ${
            ...
            />
            {!!extension && (
              <div className={InputCss.extenstionText}>{extension}</div>
            )}
          </div>
          {!!desc && (
        ...
        {!!htmlDesc && (
          <Description className={InputCss.desc}>
