...
const getFormattedValue = (value = '', customRegex, regexToValidate) =>
  customRegex
    ? value.replace(customRegex, '')
    ...
    useEffect(() => {
      function setNumValue() {
        if (regexToValidate.test(`${value}`)) setInternalValue(value);
          ...
          setInternalValue(
            getFormattedValue(value, customRegex, regexToValidate)
          );
      }
      if (fixLength) {
      ...
      <div className={[wrapperClasses, WrapClassName].join(' ')}>
        ...
        <div className={`${className} ${InputCss.inputWrapperClass}`}>
          <div className="relative">
            <input
              className={`${InputCss.inputBlock} ${inputClassName} ${sizeCss} ${
            ...
            />
            {!!extetion && (
              <div className={InputCss.extenstionText}>{extetion}</div>
            )}
          </div>
        </div>
